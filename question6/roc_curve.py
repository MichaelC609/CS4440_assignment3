...
# SPECIFICATION: Train and test a decision tree and plot the ROC curve
...
# --> add your Python code here
X = pd.get_dummies(data_training[['Refund', 'Marital Status', 'Taxable Income']], drop_first=True)
X['Taxable Income'] = X['Taxable Income'].astype(float)

# transform the original training classes to numbers and add them to the vector y. 
# For instance Yes = 1, No = 0, so Y = [1, 1, 0, 0, ...]
# --> add your Python code here
y = np.where(data_training['Cheat'].str.strip().str.lower() == 'yes', 1, 0)
